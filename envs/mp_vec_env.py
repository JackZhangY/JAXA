...
def _worker(
    remote: mp_connection.Connection,
    parent_remote: mp_connection.Connection,
    ...
    while True:
        try:
            ...
            elif cmd == "get_norm_score":
                normalized_score = env.get_normalized_score(data)
                remote.send(normalized_score)
            elif cmd == "render":
...
class SubprocVecEnv(VecEnv):
    ...
    def get_normalized_score(self, score):
        """ Retrun Normalized scores used for D4RL """
        self.remotes[0].send(("get_norm_score", score))
        normalized_scores = self.remotes[0].recv()
